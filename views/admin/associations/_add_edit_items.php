...
<div class="panel-body">
            ...
            <div class="mtop10">
                ...
                <div class="radio radio-primary radio-inline">
                    <input type="radio" value="1" id="1" name="show_quantity_as"
                        data-text="<?= _l('association_table_quantity_heading'); ?>"
                        <?= isset($association) && $association->show_quantity_as == 1 ? 'checked' : 'checked'; ?>>
                    <label
                        for="1"><?= _l('quantity_as_qty'); ?></label>
                </div>
                <div class="radio radio-primary radio-inline">
                    <input type="radio" value="2" id="2" name="show_quantity_as"
                        data-text="<?= _l('association_table_hours_heading'); ?>"
                        <?= isset($association) && $association->show_quantity_as == 2 ? 'checked' : ''; ?>>
                    <label
                        for="2"><?= _l('quantity_as_hours'); ?></label>
                </div>
                <div class="radio radio-primary radio-inline">
                    <input type="radio" id="3" value="3" name="show_quantity_as"
                        data-text="<?= _l('association_table_quantity_heading'); ?>/<?= _l('association_table_hours_heading'); ?>"
                        <?= isset($association) && $association->show_quantity_as == 3 ? 'checked' : ''; ?>>
                    <label for="3">
                        <?= _l('association_table_quantity_heading'); ?>/<?= _l('association_table_hours_heading'); ?>
                    </label>
                </div>
            </div>
        </div>
    ...
    <div class="table-responsive s_table">
        <table class="table association-items-table items table-main-association-edit no-mtop">
            <thead>
                <tr>
                    <th></th>
                    <th width="20%" align="left"><i class="fa-solid fa-circle-exclamation tw-mr-1" aria-hidden="true"
                        ...
                        <?= _l('association_table_item_heading'); ?>
                    </th>
                    <th width="25%" align="left">
                        <?= _l('association_table_item_description'); ?>
                    </th>
                    <?php
...
$qty_heading = _l('association_table_quantity_heading');
if (isset($association) && $association->show_quantity_as == 2) {
    $qty_heading = _l('association_table_hours_heading');
} elseif (isset($association) && $association->show_quantity_as == 3) {
    $qty_heading = _l('association_table_quantity_heading') . '/' . _l('association_table_hours_heading');
}
?>
...
?>
                ...
                <?php if (isset($association) || isset($add_items)) {
                    $i               = 1;
                    $items_indicator = 'newitems';
                    if (isset($association)) {
                        $add_items       = $association->items;
                    ...
                    foreach ($add_items as $item) {
                        ...
                        if (! isset($is_proposal)) {
                            $association_item_taxes = get_association_item_taxes($item['id']);
                        } else {
                            $association_item_taxes = get_proposal_item_taxes($item['id']);
                        }
                        if ($item['id'] == 0) {
                            $association_item_taxes = $item['taxname'];
                            $manual              = true;
                        }
    ...
    <div class="col-md-8 col-md-offset-4" style="display:none">
        <table class="table text-right">
            <tbody>
                <tr id="subtotal">
                    <td><span
                            class="bold tw-text-neutral-700"><?= _l('association_subtotal'); ?>
                            :</span>
                    </td>
                    <td class="subtotal">
                    </td>
                ...
                <tr id="discount_area">
                    <td>
                        <div class="row">
                            <div class="col-md-7">
                                <span
                                    class="bold tw-text-neutral-700"><?= _l('association_discount'); ?></span>
                            </div>
                            <div class="col-md-5">
                                <div class="input-group" id="discount-total">

                                    <input type="number"
                                        value="<?= isset($association) ? $association->discount_percent : 0; ?>"
                                        class="form-control pull-left input-discount-percent<?php if (isset($association) && ! is_sale_discount($association, 'percent') && is_sale_discount_applied($association)) {
                                            echo ' hide';
                                    ...
                                    <input type="number" data-toggle="tooltip"
                                        ...
                                        value="<?= isset($association) ? $association->discount_total : 0; ?>"
                                        class="form-control pull-left input-discount-fixed<?php if (! isset($association) || (isset($association) && ! is_sale_discount($association, 'fixed'))) {
                                            echo ' hide';
                                        ...
                                        <div class="dropdown">
                                            <a class="dropdown-toggle" href="#" id="dropdown_menu_tax_total_type"
                                                ...
                                                <span class="discount-total-type-selected">
                                                    <?php if (! isset($association) || isset($association) && (is_sale_discount($association, 'percent') || ! is_sale_discount_applied($association))) {
                                                        echo '%';
                                                    } else {
...
?>
                                            ...
                                            <ul class="dropdown-menu" id="discount-total-type-dropdown"
                                                ...
                                                <li>
                                                    <a href="#" class="discount-total-type discount-type-percent<?php if (! isset($association) || (isset($association) && is_sale_discount($association, 'percent')) || (isset($association) && ! is_sale_discount_applied($association))) {
                                                        echo ' selected';
                                                    } ?>">%</a>
                                                </li>
                                                <li>
                                                    <a href="#" class="discount-total-type discount-type-fixed<?php if (isset($association) && is_sale_discount($association, 'fixed')) {
                                                        echo ' selected';
                                                    } ?>">
                ...
                <tr>
                    <td>
                        <div class="row">
                            <div class="col-md-7">
                                <span
                                    class="bold tw-text-neutral-700"><?= _l('association_adjustment'); ?></span>
                            </div>
                            <div class="col-md-5">
                                <input type="number" data-toggle="tooltip"
                                    data-title="<?= _l('numbers_not_formatted_while_editing'); ?>"
                                    value="<?php if (isset($association)) {
                                        echo $association->adjustment;
                                    } else {
                                        echo 0;
                ...
                <tr>
                    <td><span
                            class="bold tw-text-neutral-700"><?= _l('association_total'); ?>
                            :</span>
                    </td>
                    <td class="total">
                    </td>
